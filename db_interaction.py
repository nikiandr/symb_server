...
def register(db: sqlite3.Connection, nickname: str, password: str) -> dict:
    """
...
def login(db: sqlite3.Connection, nickname: str, password: str) -> tuple[dict, int]:
    """
...
def add_history(db: sqlite3.Connection, user_id: int, request: dict, response: dict):
    """
    Adds req/res session to history to DB.
    Args:
...
if __name__ == '__main__':
    ...
    data, user_id = login(conn, "nikiandr", "070801")
    print(data)
    add_history(conn, user_id, {"1": 1}, {"2": 2})
    add_history(conn, user_id, {"3": 3}, {"4": 4})
    add_history(conn, user_id, {"5": 5}, {"6": 6})
    print(read_history(conn, user_id))

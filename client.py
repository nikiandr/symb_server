    ...
    try:
            ...
            if isaccount == "n":
                ...
                while True:
                    ...
                    if result["type"] == "registration_success":
                        ...
                        print(
                            "There was some kind of error with your request. "
                            + result["result"]
                            + " Let's try again."
                        )
                        continue
        ...
        while True:
            ...
            if command_type == "derivative":
                ...
                func = str(input())
                print(
                    "Enter derivative order (variable names parted with space): ",
                    end="",
                )
                ...
                func = str(input())
                req_data = {"mode": "simplify", "expression": func}
            else:
        ...
        try:
            ...
            if isanon == "n":
                while True:
                    print(
                        "Do you want to get your all-time usage history? [Y/n]: ",
                        end="",
                    )
                    ...
                    if ishistory in ("y", "n"):
                        ...
                        continue
                if ishistory == "y":
                    cl.server.sendall(ec.encode({"type": "history_request"}))
                    history = ec.decode(cp.catp_mess_get(cl.server))
                    if history["type"] == "history_error":
                        print(
                            "There was an error in getting your history. "
                            + history["history"]
                        )
                    else:

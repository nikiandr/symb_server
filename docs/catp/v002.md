...
Third byte can contain only 2 values: 0 and 1.  0 corresponds to success, 1 - to error. These byte is mostly used in computational response type of packets to mark successful and unsuccessful operations; comparing to v0.0.1, this byte is also used in login, registration and history response.
...
| Login request          | unspecified    | nickname\|password                                           | Nickname is arbitrary string, password is hashed (in SymbServer - using bcryprt). |
| Login response         | unspecified    | empty or error description                                   | All the information needed contains in third byte.           |
| Registration request   | unspecified    | nickname\|password                                           | Nickname is arbitrary string, password is hashed (in SymbServer - using bcryprt). |
...
| History response       | unspecified    | Error text or <br />mode\|input\|result as many times as there are records in history | Each record should be separated with <CR><LF>.               |
